...
function setT() {
    ...
    let formattedToday = dd + '/' + mm + '/' + yyyy + '[' + hh + ':' + min + ':' + ss + ']';
    // let formattedToday =  dd + '/' + mm + '/' + yyyy + '[' + hh +':' + min + ':'+ s + ']';

    document.getElementById('id1').innerHTML = formattedToday;
}
const city_names =[
    'Adilabad',
        'Anantapur',
...
searchbox1.addEventListener('input',function(){
    // console.log('yo');
    let sb1=document.querySelector('#searchbox1');
    // console.log(sb1.value);
    for (let i = 0; i < city_names.length; i++) {
        if(city_names[i].match(sb1.value)){
            console.log(city_names[i]);
            var item=document.createElement('li');
            item.innerHTML=city_names[i];
            let yolo=document.getElementById('lst');
            console.log(yolo);
            yolo.appendChild(item);
        }
    }
})
